...
date: "Last updated: August 10, 2021"
output: 
  pdf_document: 
    toc: yes
---
...
Tac_corr = Tac_scale %>% 
  ...
  summarise(sumGW = sum(GWCO2, na.rm = TRUE),
            ...
            efflux = sum(CO2_efflux, na.rm = TRUE))

cor.test(y = Tac_corr$sumGW, x = Tac_corr$sumrain,   # p < 0.15, t = 0.08
         method = 'kendall') 
cor.test(y = Tac_corr$sumDICin, x = Tac_corr$sumrain,   # p < 0.01, t = 0.16
         method = 'kendall') 
cor.test(y = Tac_corr$sumNEP, x = Tac_corr$sumrain,   # p < 0.78, t = -0.01
         method = 'kendall') 
cor.test(y = Tac_corr$sumF, x = Tac_corr$sumrain,   # p < 0.37, t = 0.05
         method = 'kendall') 
cor.test(y = Tac_corr$sumDICout, x = Tac_corr$sumrain,   # p < 0.01, t = 0.16
         method = 'kendall') 

# correlate GWCO2 with pH
